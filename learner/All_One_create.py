...
def RemoveBuildEval(indices, inputTrain, outputTrain, inputTest, outputTest, model, training, testing, RemoveBat,
    ...
    lst = [str(inds).replace('[', '"').replace(']', '"'), inputTrain, outputTrain, inputTest, outputTest, model,
           training, testing]
...
def All_one_parallel(sourcePathTrain, sourcePathTest, oned, alld, packsInds, names, RemoveBat):
    ...
    for p, name, ind in zip(packsInds, names, range(len(packsInds))):
        ...
        RemoveBuildEval(p, sourcePathTrain, outPathTrain, sourcePathTest, outPathTest,
                        oned + "\\models\\" + str(ind) + ".model", oned + "\\training\\" + str(ind) + ".txt",
                        oned + "\\out\\" + str(ind) + ".txt", RemoveBat, classInd)
        outPathTrain = alld + "\\" + str(ind) + "_Appended.arff"
        outPathTest = alld + "\\" + str(ind) + "_Only.arff"
        RemoveBuildEval(reduce1, sourcePathTrain, outPathTrain, sourcePathTest, outPathTest,
                        alld + "\\models\\" + str(ind) + ".model", alld + "\\training\\" + str(ind) + ".txt",
                        alld + "\\out\\" + str(ind) + ".txt", RemoveBat, classInd)
...
if __name__ == "__main__":
    ...
    packs = ["haelstead", "g2", "g3", "methodsArticles", "methodsAdded", "hirarcy", "fieldsArticles", "fieldsAdded",
             "constructorsArticles", "constructorsAdded", "lastProcess", "simpleProcessArticles", "simpleProcessAdded",
             "bugs", "sourceMonitor", "checkStyle", "blame"]  # ,"analyzeComms"]
    ...
    Roni = [[10, 11, 8], [10, 11, 6, 7, 5, 8, 9], [10, 11, 6, 7, 5, 8, 9, 0, 1], [10, 11, 6, 7, 5, 8, 9, 0, 1, 3, 4],
            [10, 11, 6, 7, 5, 8, 9, 0, 1, 3, 4, 2], [10, 11, 6, 7, 5, 8, 9, 0, 1, 3, 4, 2, 12, 13]]
    ...
    if 1 == 2:
        ...
        indsMethods = indsFamilies(packsInds, [[packs.index(x)] for x in methods])
        All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\Methods_one", d + "\\Methods_all", indsMethods,
                         methods)
        ...
        indsMethods = indsFamilies(packsInds, [[packs.index(x)] for x in methods])
        All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\constructors_one", d + "\\constructors_all",
                         indsMethods, methods)

        methods = ["simpleProcessArticles", "simpleProcessAdded"]
        indsMethods = indsFamilies(packsInds, [[packs.index(x)] for x in methods])
        All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\simpleprocess_one", d + "\\simpleprocess_all",
                         indsMethods, methods)
        exit()

        OO = ["g2", "g3", "methodsArticles", "methodsAdded", "hirarcy", "fieldsArticles", "fieldsAdded",
              "constructorsArticles", "constructorsAdded"]
        ...
        All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\process_one", d + "\\process_all", indsMethods,
                         process)
