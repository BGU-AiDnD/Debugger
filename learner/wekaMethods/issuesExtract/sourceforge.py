...
def extract_bug_data(bug):
    return bug['ticket_num'], "", ",".join(bug['labels']), bug['assigned_to'], bug['status'], "", bug['reported_by'], \
           datetime.datetime.strptime(bug['mod_date'].split('.')[0], '%Y-%m-%d %H:%M:%S').date().strftime('%d/%m/%Y %H:%M:%S'), \
           "", bug['custom_fields'].get('_milestone', ""), "", "", "P{0}".format(bug['custom_fields'].get('_priority', "")), \
           "", bug['summary'], "", datetime.datetime.strptime(bug['created_date'].split('.')[0], '%Y-%m-%d %H:%M:%S').date().strftime('%d/%m/%Y %H:%M:%S')\
        , "", "", "", ""
