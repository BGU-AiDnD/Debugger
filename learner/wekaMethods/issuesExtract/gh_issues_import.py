...
import urllib2#.Request, urllib.error, urllib.parse
...
from weka.issuesExtract import query
...
def send_request(which, url, post_data=None):
	...
	full_url = "%s/%s" % ("%s/repos/%s" % ("https://api.github.com", "jruby/jruby"), url)
	#print full_url
	#req = urllib.request.Request(full_url, post_data)
	req = urllib2.Request(full_url, post_data)

	username ="amir9979@gmail.com" #config.get(which, 'username')
	password ="AAaa1111" #config.get(which, 'password')
