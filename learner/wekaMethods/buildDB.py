...
def commTable(bugs, commits, max):
    ...
    for com in commits_items:
        ...
        commiter_date = datetime.datetime.fromtimestamp(comm.committed_date).strftime(
            '%Y-%m-%d %H:%M:%S')
        author_date = datetime.datetime.fromtimestamp(comm.authored_date).strftime(
            '%Y-%m-%d %H:%M:%S')
        ...
        fields = (
        commit_id, bugs[j], commiter_date, committer, author_date, author, lines, dels, insertions,
        files, size, parent, reach, msg, str(comm.hexsha))
        commitsBugsDict[str(comm.hexsha)] = (
        bugs[j], commiter_date, str(comm.hexsha), str(commit_id))
        all_commits.append(fields)
        stas = stats.files
        if (j % 1000 == 0):
            print
            "iteration: " + str(j) + " file: " + str(k)
        for fileName in stas:
            ...
            insertions = tot["insertions"]
            fileTuple = (
            fileName.replace("/", "\\"), commit_id, commiter_date, lines, dels, insertions, bugs[j],
            str(comm.hexsha))
...
def commTablelight(commits):
    ...
    for commit in commits:
        ...
        fields = (
        commit_id, commit._bug_id, commiter_date, committer, author_date, author, size, parent, msg,
...
def createTables(c, add):
    c.execute(
        '''CREATE TABLE AllMethods (methodDir text, fileName text, methodName text, beginLine INT , endLine INT )''')
    if (not add):
        c.execute(
            '''CREATE TABLE commitedMethods (commit_sha text , methodDir text, fileName text, methodName text, deletions INT , insertions INT , lines INT, bugId INT, commiter_date DateTime,commitID INT)''')
        c.execute(
        ...
        c.execute(
            '''CREATE TABLE commits (ID INT, bugId INT, commiter_date DateTime , commiter text,author_date DateTime , author text  , size INT, parentID INT, message text,commit_sha text )''')
        # c.execute(
        ...
        c.execute(
            '''CREATE TABLE haelsTfiles (name text, Operators_count INT, Distinct_operators INT, Operands_count INT, Distinct_operands INT, Program_length INT, Program_vocabulary INT,Volume float, Difficulty INT, Effort float)''')
        c.execute(
...
def BuildAllOneTimeCommits(git_path, dbPath, JavaDocPath, sourceMonitorFiles, sourceMonitorMethods,
    ...
    if (not add):
        ...
        insert_values_into_table(conn, "Sourcemethods",
                                 source_Monitor.source_methods(sourceMonitorMethods))
        ...
        insert_values_into_table(conn, "checkStyleExtends",
                                 checkReport.fileRead(checkStyle, False, CodeDir))
        insert_values_into_table(conn, "JAVAfilesFix",
                                 source_Monitor.source_files(sourceMonitorFiles))
