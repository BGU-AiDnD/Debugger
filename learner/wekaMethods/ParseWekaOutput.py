...
def Parse(file):
    ...
    f = open(file, "r")
    lines = f.readlines()
    titles = ["TP Rate", "FP Rate", "Precision", "Recall", " F-Measure", "MCC", "ROC Area",
              "PRC Area"]
...
def allFamilies(d, packs, packsInds):
    ...
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\Fam_one", d + "\\Fam_all", indsMethods,
                     methods)
...
def familiesTypes(d, packs, packsInds):
    ...
    OO_new = OO_old + ["bugs"]
    names = ["OO_old", "OO_new"]
    indsMethods = indsFamilies(packsInds,
                               [[packs.index(y) for y in OO_old], [packs.index(y) for y in OO_new]])
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\OO_add_one", d + "\\OO_add_all",
                     indsMethods, names)
    ...
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\complexity_add_one",
                     d + "\\complexity_add_all",
                     indsMethods, names)
    ...
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\process_add_one",
                     d + "\\process_add_all", indsMethods,
                     names)
    OO_old = ["simpleProcessArticles"] + ["methodsArticles", "hirarcy", "fieldsArticles",
                                          "constructorsArticles"] + [
                 "haelstead", "sourceMonitor", "checkStyle"]
    OO_new = OO_old + ["bugs"]
    indsMethods = indsFamilies(packsInds,
                               [[packs.index(y) for y in OO_old], [packs.index(y) for y in OO_new]])
    names = ["old", "new"]
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\All_one", d + "\\All_all", indsMethods,
                     names)
...
if __name__ == "__main__":
    ...
    names = ["Complexity_old", "Complexity_new"]
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\complexity_add_one",
                     d + "\\complexity_add_all", indsMethods, names)
    ...
    names = ["process_old", "process_new"]
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\process_add_one",
                     d + "\\process_add_all", indsMethods, names)
    ...
    OO_old = ["haelstead", "methodsArticles", "hirarcy", "fieldsArticles", "constructorsArticles",
              "simpleProcessArticles", "sourceMonitor", "checkStyle"]
    OO_new = OO_old + ["bugs"]
    indsMethods = indsFamilies(packsInds,
                               [[packs.index(y) for y in OO_old], [packs.index(y) for y in OO_new]])
    names = ["old", "new"]
    All_one_parallel(sourcePathTrain, sourcePathTest, d + "\\add_one", d + "\\add_all", indsMethods,
                     names)
