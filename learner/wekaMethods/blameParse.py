...
def oneRecordParse(lines):
    ...
    committerTime = datetime.datetime.fromtimestamp(
        float(lines[7].split(' ')[1]))  # .strftime('%Y-%m-%d %H:%M:%S')
...
def listStats(lst, isTime):
    ...
    return len(lst), numpy.mean(lst), numpy.median(lst), numpy.var(lst), maxCheck(lst), minCheck(
        lst), p01, p02, p05, mx
...
def fileParse(file, prevVersionCommitterTime):
    prevVersionCommitterTime = time.mktime(
        time.strptime(prevVersionCommitterTime, '%Y-%m-%d %H:%M:%S'))
    lines = []
    with open(file, "r") as f:
        lines = [l.strip() for l in f.readlines()]
        f.seek(0)
        inds2 = [0] + map(lambda enm: enm[0] + 2, filter(lambda enm: enm[1].startswith('filename'),
    ...
    for i in range(len(inds2) - 1):
        commitId, committer, committerTime, previous, group = oneRecordParse(
            lines[inds2[i]:inds2[i + 1]])
    ...
    for i in range(len(commitsApproved))[1:]:
        if s == commitsApproved[i]:
            ...
            s = commitsApproved[i]
    groupsApproved.append(countS)
    ret = [name, len(set(commits)), len(comms), len(set(commitsApproved)), len(commsApproved),
           numBlobs, numPatchs, numCommits]
    ...
    ret.extend(
        list(listStats(timesApproved, True)) + [maxCheck(timesApproved) - minCheck(timesApproved)])
    ret.extend(list(countsLines(timesApproved)))
    ret.extend(list(listStats(difftimesApproved, True)) + [
        maxCheck(difftimesApproved) - minCheck(difftimesApproved)])
