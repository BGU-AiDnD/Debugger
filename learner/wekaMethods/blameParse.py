...
def fileParse(file, prevVersionCommitterTime):
    ...
    with open(file, "r") as f:
        ...
        f.seek(0)
        inds2 = [0] + map(lambda enm: enm[0] + 2,
                          filter(lambda enm: enm[1].startswith('filename'), enumerate(f.readlines())))
    if len(lines) == 0:
        return 0
    ...
    for i in range(len(commitsApproved))[1:]:
        if s == commitsApproved[i]:
            ...
            s = commitsApproved[i]
    groupsApproved.append(countS)
    ret = [name, len(set(commits)), len(comms), len(set(commitsApproved)), len(commsApproved), numBlobs, numPatchs,
           numCommits]
