...
class OO:
    ...
    def classes_features(self,c,files_dict):
        interfaces='select path, 1 from classes where superClass="Interface"'
        sqlToAttributes(["0"], c, files_dict, interfaces)
        interfaces='select path, 1 from classes where superClass="java.lang.Object"'
        sqlToAttributes(["0"], c, files_dict, interfaces)
        interfaces='select path ,exception  ,externalizable  ,abstract   ,error  ,serializable from classes'
        sqlToAttributes(["false","false","false","false","false"], c, files_dict, interfaces)

    def stat(self,lst):
        counts= Counter(lst)
        return len(lst),sum(lst),numpy.mean(lst),numpy.median(lst),numpy.var(lst),max(lst),min(lst), 1 if 1==len(counts) else 0
