...
class processFamilies:
    ...
    def get_attributes(self):
        all = [("tot_changes", "NUMERIC"), ("sum_insert", "NUMERIC"),
               ...
               ("avg_delets_nonzero", "NUMERIC"), ("tot_bugs", "NUMERIC"), ("tot_changes_last", "NUMERIC"),
               ("sum_insert_last", "NUMERIC"),
               ...
               ("change_set", "NUMERIC"), ("age", "NUMERIC"), ("age2", "NUMERIC"), ("tot_changes_bugged", "NUMERIC"),
               ("sum_insert_bugged", "NUMERIC"),
               ...
               ("p1_count", "NUMERIC"), ("p2_count", "NUMERIC"), ("p3_count", "NUMERIC"), ("p4_count", "NUMERIC"),
               ("p5_count", "NUMERIC"),
               ("p1_count_perc", "NUMERIC"), ("p2_count_perc", "NUMERIC"), ("p3_count_perc", "NUMERIC"),
               ("p4_count_perc", "NUMERIC"), ("p5_count_perc", "NUMERIC"),

               ("minor_count", "NUMERIC"), ("normal_count", "NUMERIC"), ("major_count", "NUMERIC"),
               ("enhancement_count", "NUMERIC"), ("critical_count", "NUMERIC"), ("blocker_count", "NUMERIC"),
               ("trivial_count", "NUMERIC"),
               ("minor_count_perc", "NUMERIC"), ("normal_count_perc", "NUMERIC"), ("major_count_perc", "NUMERIC"),
               ("enhancement_count_perc", "NUMERIC"), ("critical_count_perc", "NUMERIC"),
               ("blocker_count_perc", "NUMERIC"), ("trivial_count_perc", "NUMERIC"),
    ...
    def simple_get_features(self, c, files_dict, prev_date, start_date, end_date):
        ...
        disAuthors = 'select name,count(distinct author) from commitedfiles,commits where commits.ID=commitedfiles.commitid  and commits.commiter_date<="' + str(
            start_date) + '"' + '  and commits.commiter_date<="' + str(end_date) + '" group by Commitedfiles.name'
        ...
        first = 'select name, count(*),  sum(insertions),sum(deletions),Sum(case When insertions > 0 Then 1 Else 0 End),Sum(case When deletions > 0 Then 1 Else 0 End),  avg(insertions),avg(deletions), avg(case When insertions > 0 Then insertions Else Null End),avg(case When deletions > 0 Then deletions Else Null End) from commitedfiles where bugId<>0 and name not like "%test%" and commiter_date<="' + str(
            start_date) + '"' + '  and commiter_date<="' + str(end_date) + '" group by name'
    ...
    def last_get_features(self, c, files_dict, prev_date, start_date, end_date):
        ...
        disAuthors_last = 'select name,count(distinct author) from commitedfiles,commits where commits.ID=commitedfiles.commitid  and commits.commiter_date>="' + str(
            prev_date) + '"' + '  and commits.commiter_date<="' + str(start_date) + '" group by Commitedfiles.name'
        ...
        last_commit = 'select name,julianday("' + str(
            ...
            start_date) + '"' + '  and commitedfiles.commiter_date<="' + str(end_date) + '" group by Commitedfiles.name'
        self.sqlToAttributes(["0"], c, files_dict, last_commit)

        last_bug = 'select name,julianday("' + str(
            start_date) + '")-julianday(max(commiter_date)) from commitedfiles  where bugId<>0 and name not like "%test%" and commitedfiles.commiter_date<="' + str(
            start_date) + '"' + '  and commitedfiles.commiter_date<="' + str(end_date) + '" group by Commitedfiles.name'
    ...
    def bugs_get_features(self, c, files_dict, prev_date, start_date, end_date):
        for p in ['"P3"']:
            first = 'select name, count(*),  sum(insertions),sum(deletions),Sum(case When insertions > 0 Then 1 Else 0 End),Sum(case When deletions > 0 Then 1 Else 0 End),  avg(insertions),avg(deletions), avg(case When insertions > 0 Then insertions Else Null End),avg(case When deletions > 0 Then deletions Else Null End) from commitedfiles,bugs where commitedfiles.bugId=bugs.id and name not like "%test%" and bugs.Priority=' + p + ' and commiter_date<="' + str(
                start_date) + '"' + '  and commiter_date<="' + str(end_date) + '" group by name'
            self.sqlToAttributes(["0", "0", "0", "0", "0", "0", "0", "0", "0"], c, files_dict, first)
        ...
        first = 'select name, count(' + case.replace('metric', '1') + ') ,  sum(' + case.replace('metric',
            ...
            'metric',
            'deletions') + ' Else Null End) from commitedfiles,bugs where commitedfiles.bugId=bugs.id and name not like "%test%"  and commiter_date<="' + str(
            start_date) + '"' + '  and commiter_date<="' + str(end_date) + '" group by name'
        print first
        self.sqlToAttributes(["0", "0", "0", "0", "0", "0", "0", "0", "0"], c, files_dict, first)
