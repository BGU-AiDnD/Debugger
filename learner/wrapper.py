...
Usage:
    wrapper.py <config_path>
    wrapper.py <config_path> learn
...
from utilsConf import version_to_dir_name
...
def Extract_OO_features_OLD():
	for version in utilsConf.get_configuration().vers:
		...
		command = """cd /d  """ + convert_to_long_path(
			...
			utilsConf.get_configuration().docletPath) + " -filename %~nxf.xml -private -d .\Jdoc2 %f >NUL 2>NUL)"
		os.system(command)
...
def blameExecute(path, pathRepo, version):
	for root, dirs, files in os.walk(pathRepo):
		for name in files:
			...
			proc = utilsConf.open_subprocess(blame_commands, stdout=subprocess.PIPE,
			                                 stderr=subprocess.PIPE, shell=True,
			                                 cwd=convert_to_long_path(pathRepo))
			(out, err) = proc.communicate()
	...
	proc = utilsConf.open_subprocess(run_commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
	                                 shell=True, cwd=convert_to_long_path(pathRepo))
	(out, err) = proc.communicate()
...
def Extract_complexity_features():
	processes = []
	for version_path, version_name, git_version in zip(utilsConf.get_configuration().vers_paths,
	                                                   utilsConf.get_configuration().vers_dirs,
	                                                   utilsConf.get_configuration().vers):
		pathRepo = os.path.join(version_path, "repo")
		run_commands = SourceMonitorXml(utilsConf.get_configuration().workingDir, version_name,
		                                utilsConf.get_configuration().sourceMonitorEXE)
		proc = utilsConf.open_subprocess(run_commands, stdout=subprocess.PIPE,
		                                 stderr=subprocess.PIPE, shell=True)
		processes.append((proc, run_commands))
		run_commands = ["java", "-jar", utilsConf.get_configuration().checkStyle68, "-c",
		                utilsConf.get_configuration().methodsNamesXML, "javaFile", "-o",
		                "vers/checkAllMethodsData/" + version_name + ".txt", pathRepo]
		proc = utilsConf.open_subprocess(run_commands, stdout=subprocess.PIPE,
		                                 stderr=subprocess.PIPE, shell=True,
		                                 cwd=convert_to_long_path(
			                                 utilsConf.get_configuration().workingDir))
		processes.append((proc, run_commands))

		run_commands = ["java", "-jar", utilsConf.get_configuration().checkStyle57, "-c",
		                utilsConf.get_configuration().allchecks, "-r", pathRepo, "-f", "xml", "-o",
	...
	for proc, run_commands in processes:
		...
		if proc.returncode != 0:
			RuntimeWarning(
				'subprocess execution failed. args are {0}. err is {1}'.format(str(run_commands),
				                                                               err))
...
def featuresExtract():
	...
	wekaMethods.commsSpaces.create(utilsConf.get_configuration().vers_dirs,
	                               os.path.join(utilsConf.get_configuration().workingDir, "vers"))
...
def BuildWekaModel(weka, training, testing, namesCsv, outCsv, name, wekaJar):
	...
	# proc = subprocess.Popen(run_commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd=convert_to_long_path(weka))
	# proc.communicate()
...
def createBuildMLModels():
	for granularity in ['File', 'Method']:
		for buggedType in ["All", "Most"]:
			trainingFile, testingFile, NamesFile, outCsv = wekaMethods.articles.get_features(
				granularity, buggedType)
...
def calc_all_probabilities(package, reduce_function):
	if '_probability' not in package:
		...
		if '_sub_packages' in package:
			sub_packages_probabilites = map(lambda x: calc_all_probabilities(x, reduce_function),
...
def save_json_watchers(precidtion_csv):
	...
	map(lambda x: add_to_packages(packges, list(reversed(x[0].split(os.path.sep))), float(x[1])),
	    lines)
...
def test_version_create():
	src = os.path.join(utilsConf.get_configuration().workingDir, "vers",
	                   utilsConf.get_configuration().vers_dirs[-2], "repo")
...
def create_web_prediction_results():
	for buggedType, granularity in itertools.product(["All", "Most"], ["files", "methods"]):
		weka_csv = os.path.join(utilsConf.get_configuration().weka_path,
		                        "{buggedType}_out_{GRANULARITY}.csv".format(buggedType=buggedType,
		                                                                    GRANULARITY=granularity))
		prediction_csv = os.path.join(utilsConf.get_configuration().web_prediction_results,
		                              "prediction_{buggedType}_{GRANULARITY}.csv".format(
			                              buggedType=buggedType, GRANULARITY=granularity))
...
def check_distribution():
	...
	all_tags = sorted(git.Repo(utilsConf.get_configuration().gitPath).tags,
	...
	tags_dates = [datetime.datetime(1900, 1, 1, 0, 0).strftime('%Y-%m-%d %H:%M:%S')] + map(
		lambda tag: datetime.datetime.fromtimestamp(tag.commit.committed_date).strftime(
			'%Y-%m-%d %H:%M:%S'), all_tags)
	headers = ["granularity", "buggedType", "test_set_valid", "test_set_bug", "training_set_valid",
	           "training_set_bug"]
		...
		for buggedType in ["All", "Most"]:
			...
			for i, version_name in list(enumerate(tags_names))[:-1]:
				...
				files_hasBug = wekaMethods.articles.get_arff_class(dbpath, start_date, end_date,
				                                                   wekaMethods.articles.BUG_QUERIES[
					                                                   granularity][buggedType],
				                                                   wekaMethods.articles.COMPONENTS_QUERIES[
					                                                   granularity])
			...
			rows.append([granularity, buggedType] + distribution[-1] + reduce(
				lambda x, y: [x[0] + y[0], x[1] + y[1]], distribution[:-1], [0, 0]))
...
def load_prediction_file(prediction_path):
	...
	with open(prediction_path) as f:
		...
		predictions = dict(
			map(lambda line: (
			line[0].replace(".java", "").replace(os.path.sep, ".").replace("").lower(), line[1]),
			    lines))
	return predictions
...
def executeTests():
	...
	tested_repo = convert_to_long_path(
		os.path.join(utilsConf.get_configuration().workingDir, "testedVer", "repo"))
	test_runner = TestRunner(tested_repo,
	                         AmirTracer(tested_repo, utilsConf.get_configuration().amir_tracer,
	...
	for granularity in utilsConf.get_configuration().prediction_files:
		for bugged_type in utilsConf.get_configuration().prediction_files[granularity]:
			components_priors = get_components_probabilities(bugged_type, granularity, test_runner,
			                                                 tests)
			tests_details = map(
...
def get_components_probabilities(bugged_type, granularity, test_runner, tests):
	...
	with open(os.path.join(utilsConf.get_configuration().web_prediction_results,
	                       utilsConf.get_configuration().prediction_files[granularity][
		                       bugged_type])) as f:
		lines = list(csv.reader(f))[1:]
		predictions = dict(
			map(lambda line: (
...
def create_experiment(test_runner, num_instances=50, tests_per_instance=50,
                      bug_passing_probability=0.05):
	results_path = os.path.join(utilsConf.get_configuration().experiments,
...
if __name__ == '__main__':
	...
	utilsConf.configure(config_file_path, logger)
	if not os.path.exists(utilsConf.get_configuration().configuration_path):
		logger.debug("")
		shutil.copyfile(config_file_path, utilsConf.get_configuration().configuration_path)

	if utilsConf.copy_from_cache() is not None:
		logger.error("Cache is not empty. Exiting.")
		exit()

	if not is_experimenting and not is_learning:
		logger.info("Running both learning & experimenting")
		run_all()
		utilsConf.export_to_cache()

	elif is_learning:
		logger.info("Running the learning algorithm")
		wrapperLearner()
