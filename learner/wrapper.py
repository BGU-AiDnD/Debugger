...
def Extract_OO_features_OLD():
    for version in utilsConf.get_configuration().vers:
        ...
        command = """cd /d  """ + utilsConf.to_short_path(
            verPath) + " & for /R .\\repo %f in (*.java) do (call javadoc -doclet com.github.markusbernhardt.xmldoclet.XmlDoclet -docletpath " + utilsConf.to_short_path(
...
def Extract_OO_features(versPath, vers, docletPath):
    for x in vers:
        ...
        run_commands = ["javadoc", "-doclet", "com.github.markusbernhardt.xmldoclet.XmlDoclet", "-docletpath ",
                        docletPath, "-private", "-d", ".\Jdoc", "@JdocFunc.txt"]
        bads = []
        if (not os.path.exists(outPath)):
            bads = bads + OO_features_error_analyze(err)
            open(os.path.join(verPath, "JdocFunc.txt"), "wb").writelines(
                [x for x in open(os.path.join(verPath, "javaFiles.txt"), "r").readlines() if x not in bads])
            proc = utilsConf.open_subprocess(run_commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True,
                                             cwd=utilsConf.to_short_path(verPath))
...
def blameExecute(path, pathRepo, version):
    for root, dirs, files in os.walk(pathRepo):
        for name in files:
            ...
            blame_commands = ['git', 'blame', '--show-stats', '--score-debug', '-p', '--line-porcelain', '-l', version,
                              git_file_path]
            proc = utilsConf.open_subprocess(blame_commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True,
                                             cwd=utilsConf.to_short_path(pathRepo))
            (out, err) = proc.communicate()
...
def Extract_complexity_features():
    ...
    for version_path, version_name, git_version in zip(utilsConf.get_configuration().vers_paths,
        ...
        proc = utilsConf.open_subprocess(run_commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True,
                                         cwd=utilsConf.to_short_path(utilsConf.get_configuration().workingDir))
        processes.append((proc, run_commands))
...
def BuildWekaModel(weka, training, testing, namesCsv, outCsv, name, wekaJar):
    ...
    os.system("cd /d  " + utilsConf.to_short_path(weka) + " & java -Xmx2024m  -cp " + utilsConf.to_short_path(
...
def createBuildMLModels():
    ...
    for buggedType in ["All", "Most"]:
        ...
        BuildWekaModel(utilsConf.get_configuration().weka_path, trainingFile, testingFile, NamesFile, outCsv,
                       "files_" + buggedType, utilsConf.get_configuration().wekaJar)
        ...
        BuildWekaModel(utilsConf.get_configuration().weka_path, trainingFile, testingFile, NamesFile, outCsv,
                       "methods_" + buggedType, utilsConf.get_configuration().wekaJar)
...
def test_version_create():
    src = os.path.join(utilsConf.get_configuration().workingDir, "vers", utilsConf.get_configuration().vers_dirs[-2],
                       "repo")
...
def create_web_prediction_results():
    for buggedType, granularity in itertools.product(["All", "Most"], ["files", "methods"]):
        ...
        prediction_csv = os.path.join(utilsConf.get_configuration().web_prediction_results,
                                      "prediction_{buggedType}_{GRANULARITY}.csv".format(buggedType=buggedType,
                                                                                         GRANULARITY=granularity))
        ...
        for buggedType in ["All", "Most"]:
            ...
            for i, version_name in list(enumerate(utilsConf.get_configuration().vers_dirs))[:-1]:
                dbpath = os.path.join(utilsConf.get_configuration().db_dir,
                                      str(utilsConf.get_configuration().vers_dirs[i] + ".db"))
            ...
            rows.append([granularity, buggedType] + distribution[-1] + reduce(lambda x, y: [x[0] + y[0], x[1] + y[1]],
                                                                              distribution[:-1], [0, 0]))
...
def load_prediction_file(prediction_path):
    ...
    with open(prediction_path) as f:
        lines = list(csv.reader(f))[1:]
        predictions = dict(
            map(lambda line: (line[0].replace(".java", "").replace(os.path.sep, ".").replace("").lower(), line[1]),
                lines))
    return predictions
        ...
        for bugged_type in utilsConf.get_configuration().prediction_files[granularity]:
            ...
            tests_details = map(
                lambda test_name: (test_name, list(
                    set(test_runner.tracer.traces[test_name].get_trace(granularity)) & set(components_priors.keys())),
            ...
            with open(diagnoses_json_path.format(granularity, bugged_type), "wb") as diagnosis_json:
                diagnosis_json.writelines(json.dumps(
                    map(lambda d: dict([('_name', d[0])] + d[1].as_dict().items()), enumerate(named_diagnoses))))
    return test_runner
...
def create_experiment(test_runner, num_instances=50, tests_per_instance=50, bug_passing_probability=0.05):
    ...
    for granularity in utilsConf.get_configuration().prediction_files:
        for bugged_type in utilsConf.get_configuration().prediction_files[granularity]:
            eg = ExperimentGenerator(test_runner, granularity, bugged_type, num_instances, tests_per_instance,
                                     bug_passing_probability)
