...
def Experiments(workingDir, packsPath, utilsPath, randNum):
...
def wrapperExperiments():
    testDb = Agent.bugs_testsDBMethods.basicBuild(utilsConf.get_configuration().workingDir,
                                                utilsConf.get_configuration().vers_dirs[-2],
                                                utilsConf.get_configuration().dates[-2],
                                                utilsConf.get_configuration().dates[-1])
    packsPath = os.path.join(utilsConf.get_configuration().workingDir, "packs.txt")
    Agent.experimentsMethods.packFileCreate(testDb, packsPath)
    for i in xrange(10):
        Experiments(utilsConf.get_configuration().workingDir, packsPath, utilsConf.get_configuration().utilsPath, i)
