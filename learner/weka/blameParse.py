...
def listStats(lst,isTime):
    ...
    if isTime:
        return len(lst),stamp(numpy.mean(lst)),stamp(numpy.median(lst)),numpy.var(lst),stamp(max(lst)),stamp(min(lst)),p01,p02,p05,mx
    return len(lst),numpy.mean(lst),numpy.median(lst),numpy.var(lst),max(lst),min(lst),p01,p02,p05,mx
...
def fileParse(file, prevVersionCommitterTime):
    prevVersionCommitterTime=time.mktime(time.strptime(prevVersionCommitterTime,'%Y-%m-%d'))
    f=open(file,"r")
    ...
    numBlobs=int(lines[-3].split(' ')[-1])
    numPatchs=int(lines[-2].split(' ')[-1])
    numCommits=int(lines[-1].split(' ')[-1])
    ...
    ret.extend(list(listStats(timesApproved,True))+[max(timesApproved)- min(timesApproved)])
    ret.extend(list(countsLines( timesApproved)))
    ret.extend(list(listStats(difftimesApproved,True))+[max(difftimesApproved)- min(difftimesApproved)])
