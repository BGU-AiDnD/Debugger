...
import Bug
from utils.monitors_manager import MonitorsManager
...
class ExperimentGenerator(object):
	def __init__(self, test_runner, granularity, bugged_type, num_instances=50,
	             tests_per_instance=50, bug_passing_probability=0.1):
		...
		self.components_priors = ExperimentGenerator.get_components_probabilities(bugged_type,
		                                                                          granularity,
		                                                                          test_runner,
		                                                                          test_runner.get_tests())
		self.bugs = filter(
			lambda bug: len(bug.get_buggy_components(self.granularity, self.bugged_type)) > 0,
			Bug.get_bugs_from_db(os.path.join(self.monitors_manager.db_dir,
			                                  self.monitors_manager.vers_dirs[-2]) + ".db",
			                     self.monitors_manager.dates[-2],
			                     self.monitors_manager.dates[-1]))
		assert len(self.bugs) > 0

	def sample_observation(self, trace, bugs):
		return 0 if random.random() < (
					self.bug_passing_probability ** len(set(trace) & set(bugs))) else 1

	def create_instances(self):
		MATRIX_PATH = os.path.join(self.monitors_manager.experiments,
		...
		while i < self.num_instances:
			...
			tests = reduce(set.__or__,
			               map(lambda x: self.test_runner.get_packages_tests().get(
			                   ...
			                   map(lambda file_name: file_name.replace('.java', '').split('.'),
			                       buggy_components)),
			               set())
			...
			for test_name in relevant_tests:
				trace = list(set(
					self.test_runner.tracer.traces[test_name].get_trace(self.granularity)) & set(
					self.components_priors.keys()))
				tests_details.append(
					(test_name, trace, self.sample_observation(trace, buggy_components)))
			if sum(map(lambda x: x[2], tests_details)) == 0:
				continue
			matrix = MATRIX_PATH.format(ITERATION=i, BUG_ID=bug.bug_id,
			...
			write_planning_file(matrix, list(buggy_components),
			                    ...
			                    description=DESCRIPTION.format(BUG_ID=bug.bug_id,
			...
			print "created instance num {ITERATION} with bugid {BUG_ID} for granularity {GRANULARITY} and type {BUGGED_TYPE}". \
				format(ITERATION=i, BUG_ID=bug.bug_id, GRANULARITY=self.granularity,
			           BUGGED_TYPE=self.bugged_type)
			i += 1
	...
	def get_components_probabilities(bugged_type, granularity, test_runner, tests):
		...
		with open(os.path.join(monitors_manager.web_prediction_results,
		                       monitors_manager.prediction_files[granularity][
			                       bugged_type])) as f:
		...
		components = set(reduce(list.__add__,
		                        map(lambda test_name: test_runner.tracer.traces[
			                        test_name].get_trace(granularity), tests), []))
		...
		for component in components:
			...
			if index < len(sorted_list):
				if sorted_list[index].startswith(reversed_component):
					components_priors[component] = max(float(predictions[sorted_list[index][::-1]]),
					                                   0.01)
