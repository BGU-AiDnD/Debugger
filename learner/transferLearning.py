...
def buildModel(wekaDir, trainingArff, project, wekaJar):
    ...
    os.system(
        "cd /d  " + wekaDir + " & java -Xmx2024m  -cp " + wekaJar + " weka.Run " + algorithm + " -x 10 -d .\\model_" + project + ".model -t " + trainingArff + " >" + project + ".txt")


def evalModel(wekaDir, testingArff, outTxt, wekaJar, project):
    algorithm = "weka.classifiers.trees.RandomForest "
    os.system(
        "cd /d  " + wekaDir + " & java -Xmx2024m  -cp " + wekaJar + " weka.Run " + algorithm + " -l .\\model_" + project + ".model -T " + testingArff + " >" + outTxt)
