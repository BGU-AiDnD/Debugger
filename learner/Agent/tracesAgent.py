...
def traces(surefire,combined,outDir):
    tests=xmlReadTest(surefire)
    names = []
    for t in tests:
        a, b = t
        names = names + [a+"\n"]
    lines=open(combined,"rb").readlines()
    inds=[]
    for n in names:
        if n in lines:
            print(n)
            inds=inds+[lines.index(n)]
    inds=inds+[len(lines)]
    classes=[]
    for l in lines:
        sp=l.split(".")
    ...
    for n in names:
        ...
        i=names.index(n)
        with open(out, 'wb') as f:
            f.writelines(set(classes[inds[i]:inds[i+1]]))
            f.writelines(["[outcome]: \n" , str(tests[i][1])])
            f.close()


if __name__ == '__main__':
    traces("C:\GitHub\\agent\\matrix\\amir\\TEST-org.eclipse.cdt.core.suite.AutomatedIntegrationSuite.xml","C:\GitHub\\agent\\matrix\\amir\\combined.txt","C:\GitHub\\agent\\matrix\\traces\\")
