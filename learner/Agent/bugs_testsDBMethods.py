...
def createTables(dbPath):
    ...
    conn.text_factory = str
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS testsMethods (Test text, methodName text,  fileName text , name text   )''')
    c.execute('''CREATE TABLE IF NOT EXISTS tests (Test text)''')
    c.execute('''CREATE TABLE IF NOT EXISTS testsOutcomes (Test text, outcome text )''')
    c.execute('''CREATE TABLE IF NOT EXISTS testsFiles (Test text, fileName text , name text )''')
    c.execute('''CREATE TABLE IF NOT EXISTS buggedFiles (BugId INT, fileName text, fileID INT, name text )''')
    c.execute('''CREATE TABLE IF NOT EXISTS buggedFilesMostModified (BugId INT, fileName text, fileID INT, name text )''')
    c.execute('''CREATE TABLE IF NOT EXISTS buggedMethods (BugId INT, methodDir text, MethodID INT, name text )''')
    c.execute('''CREATE TABLE IF NOT EXISTS buggedMethodsMostModified (BugId INT, methodDir text, MethodID INT, name text )''')
    c.execute('''CREATE TABLE IF NOT EXISTS AllFiles (fileName text, fileID INT )''')
    conn.commit()
    conn.close()
