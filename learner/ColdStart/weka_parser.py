...
ACCURACY_PER_CLASS_HEADER = \
     ...
     pp.Suppress(
		  ...
		  pp.Optional(pp.Literal("MCC")) +
		  pp.Optional(pp.Literal("ROC Area")) +
		  pp.Optional(pp.Literal("PRC Area")) +
          pp.Literal("Class")
     ) + \
     EOL

ACCURACY_WEIGHTED_HEADER = SOL + pp.Literal("Weighted Avg.").setParseAction(lambda tokens: "weighted avg")
...
# pp.Suppress(pp.Literal("Weighted Avg.") + pp.OneOrMore(ACCURACY_WEIGHTED_ENTRY))

ACCURACY_WEIGHTED_BLOCK = ACCURACY_WEIGHTED_HEADER + pp.Group(pp.OneOrMore(ACCURACY_WEIGHTED_ENTRY))
...
WEKA_SCORES = \
     ...
     pp.OneOrMore(
          ...
          pp.Optional(ACCURACY_PER_CLASS_BLOCK + ACCURACY_WEIGHTED_BLOCK + EMPTY_LINES) + \
...
def parse_WEKA_scores (text):
     ...
     accuracy_scores = ("TP rate", "FP rate", "precision", "recall", "F-measure", "MCC", "ROC area", "PRC Area")
     ...
     for i in range(0, len(data), 2):
          ...
          elif (block_name == "weighted avg"):
			  assert (performance_on_ != None)  ###
			  results[performance_on_]["accuracy by class"]["weighted avg"] = dict(zip(accuracy_scores, block_data))
...
#txtFile = open('C:\\Users\\USER\\Desktop\\allFiles\\New folder\\trainingfiles_All.txt',"r")
#a = txtFile.read()

#result = parse_WEKA_predictions(a)
#holdOn = "hi"

#a = parse_WEKA_predictions('D:\\Debbuger\\accumuloWORKING25\\weka\\testingmethods_Most.txt')
